...
var websock;var websockConnected=false;var pingTimeout;var pingInterval;const pingIntervalMS=20000;var autoReconnectInterval;function requestOrientationPermission(){}
...
function conStatusError(){websockConnected=false;clearInterval(pingInterval);$("#conStatus").removeClass("color-green");$("#conStatus").addClass("color-red");$("#conStatus").html("Error / No Connection &#8635;");$("#conStatus").off();$("#conStatus").on({click:restart,});autoReconnectInterval=setInterval(function(){if(!websockConnected)restart();},pingIntervalMS*2);}
function handleVisibilityChange(){if(!websockConnected&&!document.hidden){restart();}}
function sendPing(){websock.send("ping");pingTimeout=setTimeout(()=>{console.log("Terminating connection due to ping timeout");websock.close();clearTimeout(pingTimeout);conStatusError();},pingIntervalMS+1000);}
function start(){document.addEventListener("visibilitychange",handleVisibilityChange,false);if(window.location.port!=""||window.location.port!=80||window.location.port!=443){websock=new WebSocket("ws://"+window.location.hostname+":"+window.location.port+"/ws");}else{websock=new WebSocket("ws://"+window.location.hostname+"/ws");}
websock.onopen=function(evt){console.log("websock open");$("#conStatus").addClass("color-green");$("#conStatus").text("Connected");websockConnected=true;clearInterval(autoReconnectInterval);pingInterval=setInterval(sendPing,pingIntervalMS);};websock.onclose=function(evt){console.log("websock close");conStatusError();};websock.onerror=function(evt){console.log(evt);conStatusError();};var handleEvent=function(evt){if(evt.data.startsWith("pong")){clearTimeout(pingTimeout);return;}
var data=JSON.parse(evt.data);var e=document.body;var center="";switch(data.type){case UI_INITIAL_GUI:$("#row").html("");$("#tabsnav").html("");$("#tabscontent").html("");if(data.sliderContinuous){sliderContinuous=data.sliderContinuous;}
