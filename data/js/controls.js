...
const UI_CHART_SAVE = 183;
const UI_CHART_LOAD = 184;
      ...
      */
      ...
      case UI_TAB:
          ...
          tabs = $(".tabscontent").tabbedContent({ 
            loop: true,
            onSwitch: function() {for (const [_, ch] of Object.entries(charts)) ch.update();}
          }).data("api");
          // switch to tab...
          $("a")
            .filter(function () {
      ...
      case UI_CHART_ADD:
        if(charts.hasOwnProperty('ch' + data.id)) {
          if(data.hasOwnProperty('timeFormat')) {
            charts['ch' + data.id].data.labels.push(strftime(data.timeFormat));
          } else {
            charts['ch' + data.id].data.labels.push(data.label);
          }
          charts['ch' + data.id].data.series[0].push(data.value);

          if(data.hasOwnProperty('maxVals')) {
            while(charts['ch' + data.id].data.labels.length > data.maxVals) {
              charts['ch' + data.id].data.labels.shift();
              charts['ch' + data.id].data.series[0].shift();
            }
          }
      ...
      case UI_CHART_SAVE:
        if(charts.hasOwnProperty('ch' + data.id)) {
          localStorage.setItem("espuichart"+data.id, JSON.stringify(charts['ch' + data.id].data));
        }
        break;

      case UI_CHART_LOAD:
        if(charts.hasOwnProperty('ch' + data.id)) {
          var rv = localStorage.getItem("espuichart"+data.id);
          if (rv != null) {
            charts['ch' + data.id].data = JSON.parse(rv);
            charts['ch' + data.id].update();
          }
        }
        break;
